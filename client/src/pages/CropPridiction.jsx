...
const CropPridiction = () => {
    ...
    const fields = [
        ...
        { name: 'temperature',localname:"तापमान", placeholder: 'Temperature (°C)' },
        { name: 'humidity',localname:"आर्द्रता", placeholder: 'Humidity (g/kg)' },
        { name: 'ph',localname:"ph पातळी", placeholder: 'pH Level' },
        { name: 'rainfall',localname:"पावसाचे प्रमाण", placeholder: 'Rainfall (mm)' },
    ];
  return (
    <div className=" bg-white">
      <div className=" h-[100dvh] font-Grifter pt-48 max-w-7xl  mx-auto items-center  ">
        <div className="  flex-col flex items-center justify-center px-4">
          ...
          <div className="text-xs sm:text-sm lg:text-[30px] font-normal py-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {fields.map((field, index) => (
                <div key={index} className="flex flex-col">
                    <label htmlFor={field.name} className="mb-2 capitalize">{field.name} ({field.localname})</label>
                    <input
                        type="number"
                        name={field.name}
                        id={field.name}
            ...
            <button type="submit" className="bg-blue-500 w-1/3 text-white py-2 px-4 rounded-lg mt-4 hover:bg-blue-700">
                Submit
            </button>
            </div>
        </form>
